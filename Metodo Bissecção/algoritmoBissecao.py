...
def avaliar_expressao(): # Função para chamar e calcular a função, foi necessária pois foi usada no botão de calcular
    fx = fx_entrada.get()
    a = float(a_entrada.get())
    b = float(b_entrada.get())
    precisao = float(precisao_entrada.get())
    ...
    while math.fabs(b - a) > precisao: #calculo valor absoluto da função em modulo
        ...
        amplitude = b - a #calculo de nova aproximação
        cont = cont + 1
        if cont >= Ni:
            break
...
'''Bem vindo ao algoritmo de método da bisseção!
